...
from keyboards.inline import get_callback_btns, get_inlineMix_btns
...
async def admin_features(message: types.Message):
    await message.answer("Рад для Вас служить?", reply_markup=ADMIN_KEYBOARD)
...
async def starring_at_product(callback: types.CallbackQuery, session: AsyncSession):
    ...
    for product in await orm_get_products_list(session, int(category_id)):
...
async def back_step_handler(message: types.Message, state: FSMContext) -> None:
    ...
    if current_state == AddProduct.name:
        await message.answer(
            'Предыдущего шага нет, напишите "отмена"'
        )
        return None
...
async def add_name(message: types.Message, state: FSMContext):
    if message.text == "." and AddProduct.product_for_change:
        ...
        if 4 >= len(message.text) >= 128:
            await message.answer(
                "Название товара не должно превышать 128 символов\nили быть менее 5ти символов. \n Введите заново"
            )
            return
...
async def category_choice(callback: types.CallbackQuery, state: FSMContext , session: AsyncSession):
    if int(callback.data) in [category.id for category in await orm_get_categories(session)]:
        ...
        await callback.message.answer('Выберите катеорию.')
        await callback.answer()

@admin_router.message(AddProduct.category)
async def category_choice2(message: types.Message, state: FSMContext):
    await message.answer('Выберите катеорию.') 
...
async def add_image2(message: types.Message, state: FSMContext):
    await message.answer("Отправьте изображение твоара")